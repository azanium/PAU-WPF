...
namespace PAU.Documents
{
    public class PageElement : UserControl
    {
        ...
        private static string ConvertDateToString(DateTime? date)
        {
            ...
            return string.Format("{0}-{1}-{2}", date.Value.Day.ToString("D2"), date.Value.Month.ToString("D2"), date.Value.Year.ToString());
        }
        ...
        protected override void OnRender(DrawingContext dc)
        {
            ...
            {
                List<Object> icol = (List<Object>)PassengerToList(_dataGrid.Items[i] as PAUPassenger);
                List<int> iwidth = PassengerColWidth(_dataGrid.Items[i] as PAUPassenger);
